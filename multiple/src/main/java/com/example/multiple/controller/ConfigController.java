...
public class ConfigController {
    ...
    @GetMapping("/config/commentChange")
    @ResponseBody
    public Map<String, Object> getCommentChange(int id, String comment) {
        if (id > 0 && !comment.equals("")) {
            configService.getCommentChange(id, comment);
    ...
    public Map<String, Object> getDeleteConfig(@RequestParam String configCode) {
        if ( !configCode.isEmpty() ) {
            configService.getDeleteConfig(configCode);
